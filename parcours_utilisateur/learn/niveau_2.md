# Parcours Utilisateur : Niveau 2 d'une Leçon (Section /learn)

Ce document décrit étape par étape le parcours complet d'un utilisateur lorsqu'il effectue le Niveau 2 d'une leçon dans la section Apprentissage (`/learn`).

## 1. Déclenchement de la leçon (La Timeline)
- **Où se trouve l'utilisateur ?** Sur la page d'accueil de la section `/learn`.
- **Ce qu'il voit :** L'utilisateur voit le nœud de la leçon, qui affiche déjà un avancement partiel (1/10).
...
- **Action :** L'utilisateur clique sur **"Continuer"**. L'application le redirige vers l'URL de la leçon correspondante (`/lesson/ID_DE_LA_LECON?level=2`).
...
- **Types d'exercices et Objectifs (Spécificité du Niveau 2) :**
  - L'utilisateur fait face à des QCM avancés (WordMatch avec des distracteurs trompeurs) et des phrases à trous (FillInTheBlank) basiques. Il commence à voir les mots en contexte dans des phrases simples.
...
- **Ce qu'il voit (Écran de Complétion) :**
  ...
  - Un message célébrant la fin du niveau 2.
  - **Gain d'XP :** Le chiffre **+30 XP** s'affiche en grand (la récompense standard de l'application pour avoir complété le Niveau 2 pour la toute première fois de la journée).
...
- **Ce qu'il voit :**
  ...
  - **Progression Visuelle :** La bordure de la carte de la leçon (`SegmentedProgressBorder`) affiche désormais 2/10 segments colorés.
- **Prochaine Action possible :**
  - S'il clique à nouveau sur la leçon, la carte se déploie. Le bouton affiche toujours **"Continuer"** pour l'inviter à démarrer le Niveau 3.