# Parcours Utilisateur : Niveau 4 d'une Leçon (Section /learn)

Ce document décrit étape par étape le parcours complet d'un utilisateur lorsqu'il effectue le Niveau 4 d'une leçon dans la section Apprentissage (`/learn`).

## 1. Déclenchement de la leçon (La Timeline)
- **Où se trouve l'utilisateur ?** Sur la page d'accueil de la section `/learn`.
- **Ce qu'il voit :** L'utilisateur voit le nœud de la leçon, qui affiche déjà un avancement partiel (3/10).
...
- **Action :** L'utilisateur clique sur **"Continuer"**. L'application le redirige vers l'URL de la leçon correspondante (`/lesson/ID_DE_LA_LECON?level=4`).
...
- **Types d'exercices et Objectifs (Spécificité du Niveau 4) :**
  - L'utilisateur travaille principalement sur la construction de phrases (Sentence Builder). La difficulté augmente car des mots parasites (distracteurs) sont introduits parmi les étiquettes de mots.
...
- **Ce qu'il voit (Écran de Complétion) :**
  ...
  - Un message célébrant la fin du niveau 4.
  - **Gain d'XP :** Le chiffre **+30 XP** s'affiche en grand (la récompense standard de l'application pour avoir complété le Niveau 4 pour la toute première fois de la journée).
...
- **Ce qu'il voit :**
  ...
  - **Progression Visuelle :** La bordure de la carte de la leçon (`SegmentedProgressBorder`) affiche désormais 4/10 segments colorés.
- **Prochaine Action possible :**
  - S'il clique à nouveau sur la leçon, la carte se déploie. Le bouton affiche toujours **"Continuer"** pour l'inviter à démarrer le Niveau 5.