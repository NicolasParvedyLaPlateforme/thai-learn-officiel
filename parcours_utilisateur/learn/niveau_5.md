# Parcours Utilisateur : Niveau 5 d'une Leçon (Section /learn)

Ce document décrit étape par étape le parcours complet d'un utilisateur lorsqu'il effectue le Niveau 5 d'une leçon dans la section Apprentissage (`/learn`).

## 1. Déclenchement de la leçon (La Timeline)
- **Où se trouve l'utilisateur ?** Sur la page d'accueil de la section `/learn`.
- **Ce qu'il voit :** L'utilisateur voit le nœud de la leçon, qui affiche déjà un avancement partiel (4/10).
...
- **Action :** L'utilisateur clique sur **"Continuer"**. L'application le redirige vers l'URL de la leçon correspondante (`/lesson/ID_DE_LA_LECON?level=5`).
...
- **Types d'exercices et Objectifs (Spécificité du Niveau 5) :**
  - L'utilisateur s'entraîne sur des constructions de phrases plus complexes et des traductions inversées. L'accent est mis sur la mémorisation auditive et visuelle avec des distracteurs phonétiquement similaires.
...
- **Ce qu'il voit (Écran de Complétion) :**
  ...
  - Un message célébrant la fin du niveau 5.
  - **Gain d'XP :** Le chiffre **+30 XP** s'affiche en grand (la récompense standard de l'application pour avoir complété le Niveau 5 pour la toute première fois de la journée).
...
- **Ce qu'il voit :**
  ...
  - **Progression Visuelle :** La bordure de la carte de la leçon (`SegmentedProgressBorder`) affiche désormais 5/10 segments colorés.
- **Prochaine Action possible :**
  - S'il clique à nouveau sur la leçon, la carte se déploie. Le bouton affiche toujours **"Continuer"** pour l'inviter à démarrer le Niveau 6.