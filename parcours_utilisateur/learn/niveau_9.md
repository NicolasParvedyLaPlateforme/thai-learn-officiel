# Parcours Utilisateur : Niveau 9 d'une Leçon (Section /learn)

Ce document décrit étape par étape le parcours complet d'un utilisateur lorsqu'il effectue le Niveau 9 d'une leçon dans la section Apprentissage (`/learn`).

## 1. Déclenchement de la leçon (La Timeline)
- **Où se trouve l'utilisateur ?** Sur la page d'accueil de la section `/learn`.
- **Ce qu'il voit :** L'utilisateur voit le nœud de la leçon, qui affiche déjà un avancement partiel (8/10).
...
- **Action :** L'utilisateur clique sur **"Continuer"**. L'application le redirige vers l'URL de la leçon correspondante (`/lesson/ID_DE_LA_LECON?level=9`).
...
- **Types d'exercices et Objectifs (Spécificité du Niveau 9) :**
  - L'utilisateur est testé presque exclusivement via l'écriture libre (Free Typing) et la traduction complète. Il doit pouvoir formuler les phrases et mots de la leçon de tête.
...
- **Ce qu'il voit (Écran de Complétion) :**
  ...
  - Un message célébrant la fin du niveau 9.
  - **Gain d'XP :** Le chiffre **+100 XP** s'affiche en grand (la récompense standard de l'application pour avoir complété le Niveau 9 pour la toute première fois de la journée).
...
- **Ce qu'il voit :**
  ...
  - **Progression Visuelle :** La bordure de la carte de la leçon (`SegmentedProgressBorder`) affiche désormais 9/10 segments colorés.
- **Prochaine Action possible :**
  - S'il clique à nouveau sur la leçon, la carte se déploie. Le bouton affiche toujours **"Continuer"** pour l'inviter à démarrer le Niveau 10.