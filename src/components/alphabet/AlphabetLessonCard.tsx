...
import { AlphabetLessonDef, formatCombiningChar } from "@/lib/alphabet-utils";

interface AlphabetLessonCardProps {
  lesson: AlphabetLessonDef;
  level: number;
  unit: any;
  language: string;
...
export function AlphabetLessonCard({ lesson, level, unit, language, isReviewLocked, suggestedLessonId, onClick, maxLevelPerLesson = 4, isMobileLayout = false }: AlphabetLessonCardProps) {
  ...
  if (isMobileLayout) {
    return (
      ...
      >
        ...
        {/* Middle: Image Left, Letters Right */}
        <div className="flex flex-row gap-3 w-full items-stretch mt-1">
          {/* Left: Letters (instead of image) */}
          <div className="w-[84px] h-[84px] sm:w-[96px] sm:h-[96px] rounded-xl overflow-hidden shrink-0 relative bg-slate-100 border border-slate-200 flex items-center justify-center text-3xl font-thai text-slate-600">
          ...
          <div className="flex-1 min-w-0 bg-slate-50 rounded-xl p-1.5 border border-slate-100 flex flex-col cursor-default" onClick={(e) => e.stopPropagation()} onPointerDown={(e) => e.stopPropagation()} onMouseDown={(e) => e.stopPropagation()} onTouchStart={(e) => e.stopPropagation()}>
            <div className="flex bg-slate-200/50 p-[3px] rounded-lg w-full mb-1 justify-center">
                <span className="text-[10px] font-bold text-slate-500 py-1">{getTranslation('auto.letters', language) || "Lettres"}</span>
            </div>
            <div className="flex flex-col gap-1 overflow-y-auto min-h-[50px] max-h-[64px] px-1 no-scrollbar">
                {lesson.items.slice(0, 3).map((p: any, i: number) => (
                  <div key={i} className="flex items-center justify-between text-[10px] border-b border-slate-200/50 last:border-0 pb-0.5 last:pb-0 gap-1">
                      <span className="font-thai text-sm font-bold text-slate-700">{formatCombiningChar(p.letter)}</span>
                      <div className="flex flex-col text-right min-w-0">
                         <span className="font-bold text-slate-700 truncate">{p.pronunciation}</span>
                         <span className="text-slate-500 text-[9px] truncate">{getLocalizedField(p, 'exampleTranslation', language)}</span>
                      </div>
                  </div>
                ))}
            </div>
        ...
        <div className="flex items-center justify-between w-full mt-1 gap-3">
          ...
          <button 
            className={`px-4 py-2 sm:px-5 sm:py-2.5 rounded-xl font-extrabold text-xs sm:text-sm text-white transition-all active:scale-95 shrink-0 ${unit.bgClass || unit.colorClass} ${isReviewLocked ? 'opacity-50 pointer-events-none' : ''}`}
            onClick={(e) => {
                e.stopPropagation();
                onClick();
            }}
    ...
    >
      ...
      <div className="w-full bg-slate-50 rounded-2xl p-2 border border-slate-100 mt-2 flex flex-col gap-3 cursor-default" onClick={(e) => e.stopPropagation()} onPointerDown={(e) => e.stopPropagation()} onMouseDown={(e) => e.stopPropagation()} onTouchStart={(e) => e.stopPropagation()}>
         ...
         <div className="flex bg-slate-200/50 p-1 rounded-xl w-full justify-center">
            <span className="text-xs font-bold text-slate-500 py-1.5 px-4">{getTranslation('auto.letters', language) || "Lettres"}</span>
         </div>

         {/* Content */}
         <div className="flex flex-col gap-2 px-2 pb-2 min-h-[80px]">
            {lesson.items.map((p: any, i: number) => (
              <div key={i} className="flex items-center text-sm border-b border-slate-200/50 last:border-0 pb-2 last:pb-0 gap-3">
                  <span className="font-thai text-2xl font-bold text-slate-700 w-8 text-center">{formatCombiningChar(p.letter)}</span>
                  <div className="flex flex-col gap-0.5">
                     <span className="font-bold text-slate-700">{p.pronunciation}</span>
                     <span className="text-slate-500 text-xs">{getLocalizedField(p, 'exampleTranslation', language)}</span>
                  </div>
              </div>
            ))}
         </div>
