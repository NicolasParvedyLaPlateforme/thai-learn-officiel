...
interface PageLoadingSkeletonProps {
    ...
    timelineLineLeftMobile?: string;
    timelineLineLeftDesktop?: string;
    timelineLineDesktopWidth?: string;
}

export default function PageLoadingSkeleton({
    heroHeight = "h-[300px]",
    progressSegments = 10,
    timelineLineLeftMobile = "left-[1.25rem] sm:left-[1.5rem]",
    timelineLineLeftDesktop = "left-[calc(3.5rem-5px)] md:left-[calc(5rem-5px)]",
    timelineLineDesktopWidth = "w-[10px]"
}: PageLoadingSkeletonProps) {
    return (
        <div className="min-h-screen bg-[#FAFAFA] w-full flex flex-col md:flex-row items-start pt-[3.75rem] md:pt-8 px-0 md:px-8">
            <span className="sr-only">Chargement de l{"'"}interface...</span>
            ...
            <div className="md:hidden flex flex-col w-full px-4 mt-2">
                ...
                <div className="flex flex-col relative w-full pb-20 mt-8">
                    <div className={`absolute ${timelineLineLeftMobile} top-0 bottom-0 w-[8px] -translate-x-1/2 bg-slate-200 rounded-full z-0 animate-pulse`}></div>
                    {[...Array(5)].map((_, i) => (
            ...
            <div className="hidden md:flex flex-1 justify-center w-full pt-8 pb-32 px-6 lg:px-8 pr-8 xl:pr-12">
                <div className="flex flex-col gap-10 w-full max-w-4xl">
                    <div className="flex flex-col gap-8 w-full">
                        <div className={`p-8 md:p-10 bg-slate-200 border-b-[6px] border-slate-300 rounded-3xl ${heroHeight} w-full flex flex-col justify-between overflow-hidden relative`}>
                        ...
                        <div className="flex flex-col relative w-full pl-6 md:pl-10 mt-4 pb-32">
                            <div className={`absolute ${timelineLineLeftDesktop} top-[5rem] bottom-[8rem] ${timelineLineDesktopWidth} bg-slate-200 rounded-full z-0 animate-pulse`}></div>
                            {[...Array(6)].map((_, i) => (
