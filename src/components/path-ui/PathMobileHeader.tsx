...
export default function PathMobileHeader({
  ...
  return (
    <>
      <div className="h-[calc(3.75rem+env(safe-area-inset-top))] md:hidden w-full shrink-0" aria-hidden="true" />
      <header className={`bg-[#FAFAFA]/95 backdrop-blur-sm z-[70] h-[calc(3.75rem+env(safe-area-inset-top))] pt-[env(safe-area-inset-top)] md:hidden fixed top-0 left-0 right-0 transition-transform duration-300 ${showHeader ? 'translate-y-0' : '-translate-y-full'}`}>
